...
router.post('/user-data', checkAuth, async (req, res) => {
    ...
    if (userDetails) {
        ...
        if (req.file) {
            const oldAvatarPath = userDetails.avatarUrl ? path.join(__dirname, '..', 'public', 'img', 'avatars', userDetails.avatarUrl) : null;

            userDetails.avatarUrl = req.file.filename;
...
router.post('/profile/:id/delete-avatar', async (req, res) => {
    ...
    if (userDetails.avatarUrl) {
        const filePath = path.join(__dirname, '..', 'public', 'img', 'avatars', userDetails.avatarUrl);
